...
app.post("/suppliers/newOrder", (req, res) => {
    ...
    var params = [suppOrdID, suppID, itemDetID, itemCatID, suppOrdQty, orderDate, ordReceiveDate];
  
    db.all("INSERT INTO suppOrder (suppOrdID, suppID, itemDetID, itemCatID, suppOrdQty, orderDate, ordReceiveDate) VALUES (?, ?, ?, ?, ?, ?, ?)", params, (err) => {
      if (err) {
        res.json({ error: err.message });
        return;
      }
      res.json({ message: "Order %ID%".replace("%ID%", suppOrdID) + " has been sent!" });
    });
  });
...
app.get("/suppliers/orderHistory/:suppOrdID", (req, res) => {
    ...
    db.all("SELECT * FROM suppOrder WHERE suppOrdID = ?", suppOrdID, (err, result) => {
      if (err) {
        res.json({ error: err.message });
        return;
      }
...
app.put("/suppliers/orderReceived", (req, res) => {
    ...
    db.all(
      `UPDATE suppOrder SET ordReceiveDate = ? WHERE suppOrdID = ?`, params, (err) => {
        if (err) {
