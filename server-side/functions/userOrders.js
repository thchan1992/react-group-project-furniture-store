...
app.get("/account/order/History/:basketItemID/:userID", (req, res) => {
  ...
  db.all(sql, basketItemID, (err, result) => {
    if (err) {
      res.json({ error: err.message });
      return;
    }
...
app.get("/account/userOrder/:userID", (req, res) => {
  const userID = req.params.userID;
  var sql =
    "SELECT userID, basketItemID, SUM(sales.itemPrice*itemBasketQty) AS totalCost, deliveryDate, orderDate from sales INNER JOIN itemDetails ON itemDetails.itemDetID = sales.itemDetID group by basketItemID having userID=?";
